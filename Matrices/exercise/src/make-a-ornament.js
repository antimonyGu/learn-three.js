...
function fillScene() {
	...
	cylinder  = new THREE.Mesh(
		new THREE.CylinderGeometry( 0.2, 0.2, cylLength, 32 ), cylinderMaterial );
	let secondRotationAxis = new THREE.Vector3(1, 0, 1);
	secondRotationAxis.normalize();
	let secondCylAxis = new THREE.Vector3().subVectors(new THREE.Vector3(1, -1, 1), new THREE.Vector3(-1, 1, -1));
	secondCylAxis.normalize();
	let secondTheta = Math.acos( secondCylAxis.dot( new THREE.Vector3(0,1,0) ) );
	cylinder.matrixAutoUpdate = false;
	cylinder.matrix.makeRotationAxis( secondRotationAxis, secondTheta );
	scene.add( cylinder );
