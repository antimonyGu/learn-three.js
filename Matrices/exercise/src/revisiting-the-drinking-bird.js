...
function createDrinkingBird() {
        ...
        let crossbar = new THREE.Mesh(new THREE.CylinderGeometry(5, 5, 200, 32), crossbarMaterial);
        ...
        let nose = new THREE.Mesh(new THREE.CylinderGeometry(6, 14, 70, 32), headMaterial);
        nose.position.y = 530;
        nose.position.x = -70;
        ...
        // eyes 
        let sphGeom = new THREE.SphereGeometry( 10, 32, 16 ); // left eye 
        ...
        eye.add( sphere ); eye.rotation.y = -20 * Math.PI / 180.0;
        scene.add( eye );
}
...
function drawHelpers() {
        if (ground) {
                scene.add(Coordinates.drawGround({size:10000}));
        }
        if (gridX) {
