...
function createCapsule( material, radius, top, bottom, segmentsWidth, openTop, openBottom )
{
    ...
    debugger
    let sphereTop = new THREE.Mesh(sphGeom, material);
    sphereTop.position.y += length/2;
    let sphereBottom = new THREE.Mesh(sphGeom, material);
    sphereBottom.position.y -= length/2;
    capsule.add(cyl);
    if (!openTop) capsule.add(sphereTop);
    if (!openBottom) capsule.add(sphereBottom);
