...
function createCylinderFromEnds( material, radiusTop, radiusBottom, top, bottom, segmentsWidth, openEnded)
{
	...
	////////////////////
	let cylAxis = new THREE.Vector3().subVectors(top, bottom);
	let length = cylAxis.length();
	let center = new THREE.Vector3().addVectors(top, bottom).divideScalar(2);
...
function makeLengthAngleAxisTransform( cyl, cylAxis, center )
{
	...
	if ( rotationAxis.length() < 0.000001 ) // when two vectors are parallel their crossvector is zero vector
	{
	...
	var theta = -Math.acos( cylAxis.dot( yAxis ) ); // makeRotationAxis rotate the object counterclockwise, so negative is needed.
...
function drawHelpers() {
	...
	if (gridX) {
		...
		scene.add( Coordinates.drawGrid({size:10000,scale:0.01}) );
	}
	if (gridY) {
		scene.add( Coordinates.drawGrid({size:10000,scale:0.01, orientation:"y"}));
	}
	if (gridZ) {
		scene.add(Coordinates.drawGrid({size:10000,scale:0.01, orientation:"z"}));	
	}
	if (axes) {
		scene.add(Coordinates.drawAllAxes({axisLength:200,axisRadius:1,axisTess:50}));
	}
}
