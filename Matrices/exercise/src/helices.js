...
function createHelix( material, radius, tube, radialSegments, tubularSegments, height, arc, clockwise )
{
    ...
    // TODO: allow arc product to be a floating number
	for ( var i = 0; i <= arc*radialSegments ; i++ )
	{
        ...
        if (i === 0) {
            previousPosition.copy(top);
            continue;
        }
        let openTopFlag = (i === arc*radialSegments) ? true : false;
        let openBottomFlag = (i === 1) ? true : false;
        // TODO: use instancing instead of creating a new model every time
        let capsule = createCapsule(material, tube, top, previousPosition, undefined, openTopFlag, openBottomFlag );
        previousPosition.copy(top);
        helix.add( capsule );
	}
	///////////////
