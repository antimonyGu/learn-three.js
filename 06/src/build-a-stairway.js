...
function createStairs() {
	...
	const generateSteps = (count) => {
		const yOffset = verticalStepHeight;
		const zOffset = horizontalStepDepth;
		stepMesh = new THREE.Mesh( stepVertical, stepMaterialVertical );
		// The position is where the center of the block will be put.
		// You can define position as THREE.Vector3(x, y, z) or in the following way:
		stepMesh.position.x = 0;			// centered at origin
		stepMesh.position.y = verticalStepHeight/2 + yOffset * count + stepThickness;	// half of height: put it above ground plane
		stepMesh.position.z = 0 + zOffset * count - stepThickness;			// centered at origin
		scene.add( stepMesh );
		...
		stepMesh.position.x = 0;
		// Push up by half of horizontal step's height, plus vertical step's height
		stepMesh.position.y = stepThickness/2 + verticalStepHeight + yOffset * count + stepThickness;
		// Push step forward by half the depth, minus half the vertical step's thickness
		stepMesh.position.z = horizontalStepDepth/2 - stepHalfThickness + zOffset * count - stepThickness;
		scene.add( stepMesh );
	}
	for(let i = 1; i < 6; i++)
		generateSteps(i)
...
function init() {
	...
	renderer.setClearColor(new THREE.Color(0xEEEEEE, 1.0)); // setClearColorHex has been been deprecated
	...
	window.THREE = THREE;
	cameraControls = new OrbitControls(camera, renderer.domElement); // OrbitAndPanControls has been been deprecated
...
function fillScene() {
	...
	debugger
	if (ground) {
		let groundMesh = Coordinates.drawGround({size:1000});	
		scene.add( groundMesh );	
	}
	if (gridX) {
		let gridXMesh = Coordinates.drawGrid({size:1000,scale:0.01});
		scene.add( gridXMesh );
	}
	if (gridY) {
		let gridYMesh = Coordinates.drawGrid({size:1000,scale:0.01, orientation:"y"});
		scene.add( gridYMesh );
	}
	if (gridZ) {
		let gridZMesh = Coordinates.drawGrid({size:1000,scale:0.01, orientation:"z"});	
		scene.add( gridZMesh );
	}
	if (axes) {
		let axesMesh = Coordinates.drawAllAxes({axisLength:300,axisRadius:2,axisTess:50});
		scene.add( axesMesh );
	}
	createCup();
	debugger
